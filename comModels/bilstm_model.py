...
# -----------------------------
# 文本编码（BiLSTM 基线）：支持两种输入路径
#   A) 常规：input_ids + attention_mask → Embedding → BiLSTM
#   B) 预计算：token_embeds + attention_mask (可选 lengths)
# 输出与原版保持一致：H_sent:(B,1,2h), S_doc:(B,2h), mask_sent:(B,1)
# -----------------------------
class TextModel(nn.Module):
    def __init__(self,
                 ...
                 vocab_size: int = 21128,       # 词表大小（默认中文BERT词表规模）
                 pad_id: int = 0,
                 dropout: float = 0.1,
        ...
        # BiLSTM + 注意力（与旧版保持输出维度一致：2*gru_hidden）
        self.bigru_word = nn.LSTM(self.embed_dim, gru_hidden, bidirectional=True, batch_first=True)
        self.word_att = WordAttention(2 * gru_hidden, attn_hidden)
        self.bigru_sent = nn.LSTM(2 * gru_hidden, gru_hidden, bidirectional=True, batch_first=True)

    def _pack_run_lstm(self, X: torch.Tensor, attention_mask: torch.Tensor):
        """对变长序列做 pack/pad 的 LSTM 前向，返回 H_word(B,T,2h) 与 bool mask"""
    ...
    def forward_from_embeds(
        ...
        if E != self.embed_dim:
            raise ValueError(f"[BiLSTM TextModel] 预计算向量维度 E={E} 与 embed_dim={self.embed_dim} 不一致。"
                             f"请把 config.text_embed_dim/bert_dim 设为 {E} 或调整你的 token_embeds。")

        X = self.emb_drop(token_embeds)
        H_word, mask_bool = self._pack_run_lstm(X, attention_mask)
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.text_model = TextModel(
            backbone="bilstm",
            # bert_dim 在 BiLSTM 中作为词向量维度；优先取 text_embed_dim（无则回退 text_hidden / 300）
            ...
            # 若你的 DataProcessor 使用自建词表，可在 config 里传入
    ...
    def _maybe_freeze_backbones(self, config):
        # 文本：只训练最后 n 层（若是 BERT/CLIP 时生效；BiLSTM 没有 encoder，此段将被跳过）
