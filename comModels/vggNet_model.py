...
try:
    ...
    torchvision = None
    vgg11 = vgg16 = vgg19 = None
    VGG11_Weights = VGG16_Weights = VGG19_Weights = None
...
class WordAttention(nn.Module):
    ...
    def forward(self, H: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if mask.dtype != torch.bool:
            mask = mask != 0
        score_fp32 = score.float().masked_fill(~mask, torch.finfo(score.float().dtype).min)
...
class ImageModel(nn.Module):
    ...
    def __init__(self,
        ...
        if name_l.startswith("openai") or (("clip" in name_l) and CLIPModel is not None):
            ...
            # ---- VGG 分支 ----
            if torchvision is None or vgg16 is None:
                raise ImportError("torchvision 不可用，无法加载 VGG 视觉骨干。")
            # 选择 VGG 变体
            if name_l == "vgg11":
                net = vgg11(weights=VGG11_Weights.IMAGENET1K_V1 if VGG11_Weights else None)
            elif name_l == "vgg19":
                net = vgg19(weights=VGG19_Weights.IMAGENET1K_V1 if VGG19_Weights else None)
            else:
                net = vgg16(weights=VGG16_Weights.IMAGENET1K_V1 if VGG16_Weights else None)

            # 仅取卷积特征部分
            self.cnn = net.features  # (B,512,h,w)
    ...
    def forward(self, images: torch.Tensor, roi_vec: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._use_clip_visual:
            ...
            g = self.pool(feat).flatten(1)    # (B,512)
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 图像侧：改为 VGGNet（默认 vgg16）
        self.img_model = ImageModel(
            proj_dim=getattr(config, "hdn_hidden", 512),
            backbone=getattr(config, "image_backbone", "vgg16"),
        ...
        # 冻结主干（仅当是 ResNet 时才按层名冻结；VGG 分支不受该逻辑影响）
    ...
    def _maybe_freeze_backbones(self, config):
        ...
        # 图像侧：原来只处理 ResNet 层名；VGG 不在此范围内，直接跳过
