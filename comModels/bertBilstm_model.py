...
# -----------------------------
# 文本编码（BERT → BiLSTM + 注意力）
#   - 常规：input_ids + attention_mask → BERT → BiLSTM → 注意力 → 句级BiLSTM
#   - 预计算：token_embeds + attention_mask（可选；支持 enc_dim=bert_hidden 或 enc_dim=bert_dim）
# 输出接口保持不变：
...
class TextModel(nn.Module):
    def __init__(self,
        ...
        # 编码器原始 hidden_size 与输出目标维度
        self._enc_dim = getattr(getattr(self.encoder, "config", None), "hidden_size", bert_dim)
        self._bert_out_dim = int(bert_dim)

        # 适配到 bert_dim
        self.in_proj = nn.Linear(self._enc_dim, self._bert_out_dim) if self._enc_dim != self._bert_out_dim else nn.Identity()

        # 用 BiLSTM（而非 GRU）
        self.bigru_word = nn.LSTM(self._bert_out_dim, gru_hidden, bidirectional=True, batch_first=True)
    ...
    def forward_from_embeds(
        ...
        # 若传入的是 BERT 原 hidden_size，走 in_proj；若已是 bert_dim，直接用
        if E == self._enc_dim:
            X = self.in_proj(token_embeds)
        elif E == self._bert_out_dim:
            X = token_embeds
        else:
            raise ValueError(
                f"[BERT-BiLSTM TextModel] 预计算向量维度 E={E} 不匹配，"
                f"需等于 enc_dim({self._enc_dim}) 或 bert_dim({self._bert_out_dim})."
            )
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 可选：torch.compile（变量名沿用 bigru_* 以保持与旧训练脚本兼容）
