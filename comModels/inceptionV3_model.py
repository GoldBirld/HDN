...
# -----------------------------
# 图像编码：InceptionV3 全局；ROI 由数据阶段提供
# -----------------------------
class ImageModel(nn.Module):
    """
    对外接口: forward(images, roi_vec=None) -> (P, r)
      - P: (B, proj_dim) 全局向量（CLIP/InceptionV3）
      - r: (B, proj_dim) 区域向量；若提供 roi_vec(=B×roi_in_dim) 则映射后返回；否则 r=P
    支持 backbone:
      - "inception_v3" (默认，输入按 299×299 适配)
      - "openai:clip-..." / "clip-..."（保持对 CLIP 的兼容）
    """
    def __init__(self,
                 proj_dim: int = 512,
        ...
        if name_l.startswith("openai") or (("clip" in name_l) and CLIPModel is not None):
            ...
            # ---- InceptionV3 分支 ----
            if torchvision is None or inception_v3 is None:
                raise ImportError("torchvision 不可用，无法加载 InceptionV3 视觉骨干。")

            # 关闭 AuxLogits，加载 ImageNet 预训练
            net = inception_v3(weights=Inception_V3_Weights.IMAGENET1K_V1 if Inception_V3_Weights else None,
                               aux_logits=False)
            # 取其主干做特征输出：去除分类头 & dropout
            net.fc = nn.Identity()
            if hasattr(net, "dropout"):
                net.dropout = nn.Identity()
            self.cnn = net  # 其前向将输出 (B,2048)
    ...
    def forward(self, images: torch.Tensor, roi_vec: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._use_clip_visual:
            ...
            # InceptionV3 需要 299×299
            if x.shape[-1] != self._inception_image_size or x.shape[-2] != self._inception_image_size:
                x = F.interpolate(x, size=(self._inception_image_size, self._inception_image_size),
                                  mode="bilinear", align_corners=False)
            # InceptionV3 的 forward 直接返回 (B,2048)（已将 fc/dropout 设为 Identity）
            g = self.cnn(x)                     # (B,2048)
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.img_model = ImageModel(
            ...
            backbone=getattr(config, "image_backbone", "inception_v3"),
        ...
        # （仅对 ResNet 冻结策略生效；Inception/VGG 不受 layer 名字控制，安全跳过）
    ...
    def _maybe_freeze_backbones(self, config):
        ...
        # 图像侧：原有逻辑只处理 ResNet 层名；InceptionV3 不在此范围内，直接跳过
